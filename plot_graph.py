...
def plot_graphs(path: str, T2: float):
    ...
    axes.plot(ER, n1_percents, '^-', label='C', linewidth=1)
    axes.plot(ER, n2_percents, 'o-', label=r'$H_2$', linewidth=1)
    axes.plot(ER, n3_percents, 's-', label='CO', linewidth=1)
    axes.plot(ER, n4_percents, 'x-', label='$CO_2$', linewidth=1)
    axes.plot(ER, n6_percents, 'd--', label='$CH_4$', linewidth=1)
    ...
    plt.legend(bbox_to_anchor=(0.5, -0.3), loc="lower center", ncol=5)
